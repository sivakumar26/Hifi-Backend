...
public class PostsResolver {
	...
	@POST
	@Path("{userPostId}/{userFacebookId}")
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces({ MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })

	public Response mapPostToUser(@HeaderParam("token") String token, @PathParam("postId") String postId,
		...
		if (response.getStatus() == 200) {
			System.out.println("Mapping post with post id" + postId + "to user " + userFacebookId);
			...
			if (postRepository.update(postId,"IN_PROGRESS")) {
				// Marked the post as active = false
				...
				assigneeOps.buildAssignee(userFacebookId, postId);
